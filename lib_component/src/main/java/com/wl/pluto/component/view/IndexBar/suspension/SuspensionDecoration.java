package com.wl.pluto.component.view.IndexBar.suspension;
...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private int COLOR_TITLE_BG = Color.parseColor("#FFe9e9e9");
    private int COLOR_TITLE_FONT = Color.parseColor("#FF222222");
    private int COLOR_TITLE_BLUE = Color.parseColor("#FF127ADB");
    ...
    public SuspensionDecoration(Context context, List<? extends ISuspensionInterface> datas,int titleHeight,int textSize,int padddingLeft) {
        super();
        mDatas = datas;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        paddingLeft = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, padddingLeft, context.getResources().getDisplayMetrics());
        mTitleFontSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, textSize, context.getResources().getDisplayMetrics());
        mPaint.setTextSize(mTitleFontSize);
        mPaint.setAntiAlias(true);
        mInflater = LayoutInflater.from(context);
    }
...
/*
        ...
        if(mDatas != null && !TextUtils.isEmpty(mDatas.get(position).getSuspensionTag())){
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        mPaint.setColor(COLOR_TITLE_BLUE);
        if(!TextUtils.isEmpty(tag)){
            mPaint.getTextBounds(tag, 0, tag.length(), mBounds);
        }
        c.drawText(tag, paddingLeft, parent.getPaddingTop() + mTitleHeight - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
        if (flag){
